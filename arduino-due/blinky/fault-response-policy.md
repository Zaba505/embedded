...
1. **Device & what it controls.** Arduino Due (Atmel SAM3X8E, Cortex-M3). Exactly one output with
   any effect on the world: a single externally-wired LED on `PB26`, active-high, current-limited to
   ~2 mA. It drives nothing else — no motor, heater, radio, bus, or actuator. A held-on or held-off
   LED can harm nothing.
...
4. **Rationale.** The failure this project exists to rule out is a *reset loop that looks like a
   ...
   field 1 has nothing dangerous to leave energized — the worst outcome of halting is a dark LED,
   which is the signal we want. (Full argument: the project [README](README.md), "Why not the
   on-board LED" and "Verifying it worked".)
...
5. **What counts as a fault.**
   ...
   - **Any CPU exception** — every entry in the vector table in [`src/start.zig`](src/start.zig)
     (`NMI`, hard/bus/usage fault, and the deliberately-wired-but-unused `SysTick` slot) points at
     `defaultHandler`, which halts.
...
6. **Context caveats.** The firmware is **poll-driven, not interrupt-driven**: SysTick is read via
   ...
   context* here because reaching the safe state (field 2) requires no peripheral to be up — it
   needs nothing but the CPU, so there is no init-order or masked-interrupt hazard to design around.

7. **Observability.** A **dark, unchanging LED** — the *absence* of the 1 Hz blink. There is
   deliberately no second indicator: the entire design rejects "an indicator can lie," and a stopped
   blink is a signal a reset loop cannot counterfeit. To learn *where* it halted, attach a 3.3 V SWD
   probe (see the README's debug-probe notes); the trap loops are tight `while (true) {}`, so the PC
   sits on the faulting handler.
